...
#define _MAX_NUMBER_OF_BUDGET 60

#define _MAX_NUMBER_OF_VEHICLES 25
#define _MAX_NUMBER_OF_AGENTS 200
...
int g_number_of_LR_iterations = 50;
...
int g_virtual_vehicle_path_node_sequence[_MAX_NUMBER_OF_AGENTS][_MAX_NUMBER_OF_TIME_INTERVALS];
int g_virtual_vehicle_path_time_sequence[_MAX_NUMBER_OF_AGENTS][_MAX_NUMBER_OF_TIME_INTERVALS];
int g_virtual_vehicle_path_resource_sequence[_MAX_NUMBER_OF_AGENTS][_MAX_NUMBER_OF_TIME_INTERVALS];
int g_virtual_vehicle_path_number_of_nodes[_MAX_NUMBER_OF_AGENTS];
int g_virtual_vehicle_capacity[_MAX_NUMBER_OF_AGENTS];
int g_virtual_vehicle_path_link_sequence[_MAX_NUMBER_OF_AGENTS][_MAX_NUMBER_OF_TIME_INTERVALS];
float g_virtual_vehicle_path_cost_sequence[_MAX_NUMBER_OF_AGENTS][_MAX_NUMBER_OF_TIME_INTERVALS];
...
float g_knapsack_problem(float station_capacity_price[_MAX_NUMBER_OF_NODES],float total_budget,int g_number_of_depot)
{
	...
	for(int i=0;i<g_number_of_depot_node;i++)
	{
		...
		benefit[i+1] = /* M+1 * */ station_capacity_price[g_depot_node[i]];
	...
	{
		g_status_transition_matrix[0][b]=0;
	}
...
float g_Optimization_Lagrangian_Method_Resource_Constrained_Location_Routing_Problem()
{
	...
	float totalVirtualVehicleCost=0.0;
	...
	double DemandStepSize = .8f / (LR_iteration + 1.0f);
		...
		for(int i = 0; i < g_number_of_nodes; i++)
		{
			if(g_node_type[i]==0)
			{
				...
				//g_depot_capacity_profit_for_Knapsack[i]=g_depot_capacity_profit_for_Knapsack[i] + min(0,(DemandStepSize/10) *( g_depot_capacity_usage[i] -   g_depot_capacity[i]));
				g_depot_capacity_profit_for_Knapsack[i]=max(0,(g_depot_capacity_profit_for_Knapsack[i] +max(0,(DemandStepSize/10) *( g_depot_capacity_usage[i] -   g_depot_capacity[i]))));
		...
		//
		//step 2: Lower bound, solve Py-2, multi vehicle routing problem, 
		...
		float transportation_cost_supplement=0;
	...
	/*			if(g_location_routing_debugging_flag)
		...
		//deal with unsatisfied demand in lowerbound with virtual vehicle
		//if(g_supply_to_agent_demand[i]==0)
		totalVirtualVehicleCost=0.0;
		//int link_no_virtual=-1;
		for(int i=0;i<g_number_of_agents;i++)//for all demands
		{
			if(g_supply_to_agent_demand[i] == 0)//if demand still exist after upperbound routing process
			{
				//active the virtual vechile
				bool f = true;
				float VirtualVehicleCost =
				g_optimal_resource_space_time_dynamic_programming_shortest_path
				(1,
				g_arc_cost_for_upper_bound,
				g_to_node_cost,
				g_vertex_waiting_cost,
				g_agent_origin_node[i],
				g_agent_departure_time_beginning[i],
				g_agent_departure_time_ending[i],
				g_agent_destination_node[i],
				g_agent_arrival_time_beginning[i],
				g_agent_arrival_time_ending[i],
				g_virtual_vehicle_path_number_of_nodes[i],
				g_virtual_vehicle_path_node_sequence[i],
				g_virtual_vehicle_path_link_sequence[i],
				g_virtual_vehicle_path_time_sequence[i],
				g_virtual_vehicle_path_resource_sequence[i],
				g_virtual_vehicle_path_cost_sequence[i],
				0,
				g_virtual_vehicle_capacity[i],
				g_path_travel_time_f);

						
				/*for(int n=0;n<g_virtual_vehicle_path_number_of_nodes[i]-1;n++)
				{
					if(g_virtual_vehicle_path_node_sequence[i][n]!=g_virtual_vehicle_path_node_sequence[i][n+1])
					{
						link_no_virtual=g_get_link_no_based_on_from_node_to_node(g_virtual_vehicle_path_node_sequence[i][n], g_virtual_vehicle_path_node_sequence[i][n+1]);
						if(link_no_virtual>=0)
						{
							VirtualVehicleCost += 3 * g_link_free_flow_travel_time[link_no_virtual];
						}
					}
				}*/
				totalVirtualVehicleCost += VirtualVehicleCost;
			}
		}
		...
		//deal with unsatisfied demand in lower bound, lower bound punishment
			...
			g_lower_bound[LR_iteration] += totalVirtualVehicleCost;
		...
		for(int v = 1;v <= g_number_of_required_vehicles; v++)
		{
			...
			for(int i = 0; i < g_vehicle_path_number_of_nodes[v]-1; i++)
			{
				for(int j = 0;j< g_number_of_depot_node; j++)
				{
					...
					{
						if(g_vehicle_path_node_sequence[v][i] == TemporaryStationRepeatRecord) break;
		...
		//step 3. solve SPw, Knapsack Problem
		...
		//// step 4. Upper bound calculation, solve Px-2
		//step 4.1: Initialization of UB 
		...
		//dealing with unsatisfied demand with virtual vehicle for upperbound
		totalVirtualVehicleCost=0.0;
		int link_no_virtual=-1;
		for(int i=0;i<g_number_of_agents;i++)//for all demands
		{
			if(g_agent_current_demand_for_upper_bound[i] > 0)//if demand still exist after upperbound routing process
			{
				//active the virtual vechile
				bool f=true;
				float VirtualVehicleCost =
		...
		g_upper_bound[LR_iteration] += totalVirtualVehicleCost;
		//if(g_location_routing_debugging_flag) 
		...
		//if objective function value decrease, update the gap
		//if(g_objective_function_value[LR_iteration]<g_objective_function_value[LR_iteration-1])
			g_gap[LR_iteration] = (g_upper_bound[LR_iteration] - g_lower_bound[LR_iteration]) / g_upper_bound[LR_iteration];
		if(LR_iteration>6)
		{
			//if gap does not decrease, drop the solution
			if(g_gap[LR_iteration]>g_gap[LR_iteration-1] || g_objective_function_value[LR_iteration]>g_objective_function_value[LR_iteration-1])
			{
		...
		//if(g_location_routing_debugging_flag)
		{
			...
			for(int i =5;i<_MAX_NUMBER_OF_ITERATIONS;i++)
			{
