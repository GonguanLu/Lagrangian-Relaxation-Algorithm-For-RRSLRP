...
//void g_DynamicProgramming(int k)  // train index k
//{
//	// initialization 
//
//	for (int i = 0; i < _MAX_NUMBER_OF_CELLS; i++)
...
//
//
//					if ((g_TrainStationFlag[k] == 1 && v == 0)  /*stop statsion, only allow v= 0 */
//						|| (g_TrainStationFlag[k] == 0 && (v >= 1 && v < _MAX_MOVE_STEPS && (i + j_p < _MAX_NUMBER_OF_CELLS)) ) )  /*non-stop statsion, only allow v >= 1 */
//					{
//						float NewLabelCost = g_LabelCost[i][t][u] + g_TrainArcCost[i][j_p][u][v];
//
