...
public:
	...
	template <class T> bool GetValueByFieldName(string field_name, T& value, bool NonnegativeFlag = false)
	{
		if (FieldsIndices.find(field_name) == FieldsIndices.end())
		{
			return false;
...
public:
	...
	wchar_t * chTest ; //¶¯Ì¬ÉêÇë¿Õ¼ä

	void WriteTextString(CString textString)
	{
		if (!outFile.is_open())
			return;
		chTest = new wchar_t[textString.GetLength()+1];
		chTest = textString.GetBuffer(0); //»ñÈ¡CStringÄÚÈÝµÄµØÖ·
		for(int i = 0; i<textString.GetLength();i++)
		{
			outFile<<(char)*(chTest+i);
		}
		textString.ReleaseBuffer();
		outFile << endl;
